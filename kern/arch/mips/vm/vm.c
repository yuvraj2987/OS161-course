		...
		if(coreMap[count].state==FREE)
		{
			...
			if(foundContinuousPages==1)
			{
				...
				for(innerCount=count; innerCount<(count+npages); innerCount++)
				{
					...
					coreMap[innerCount].pageCount = (npages -(innerCount-count));
				}
				return returnAdd;
			}
...
int vm_fault(int faulttype, vaddr_t faultaddress)
{
	...
	while(1)
	{
		KASSERT((page_table_list->as_virtual & PAGE_FRAME)==page_table_list->as_virtual);
		...
		if(page_table_list->as_virtual==faultaddress)
		{
			...
			page_table_list->as_physical = get_user_pages(1, as, page_table_list->as_virtual);
